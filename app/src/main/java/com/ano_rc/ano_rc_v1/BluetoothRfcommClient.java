...
public class BluetoothRfcommClient {
    ...
    //public static final int STATE_LISTEN = 1;     // now listening for incoming connections 定义蓝牙连接的一些变量
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        mConnectedThread.start();//启动连接程序

        // Send the name of the connected device back to the UI Activity
        Message msg = mHandler.obtainMessage(MainActivity.MESSAGE_DEVICE_NAME);//传进来从主页面获得的蓝牙设备的名称
        Bundle bundle = new Bundle();
        bundle.putString(MainActivity.DEVICE_NAME, device.getName());
        msg.setData(bundle);
        mHandler.sendMessage(msg);

        setState(STATE_CONNECTED);//把状态改为已连接
    }
    ...
    private class ConnectThread extends Thread {
        private final BluetoothSocket mmSocket;//接口
